---
description: 进入 explore 模式，思考想法、调查问题并澄清需求
---
...
**输入**：`/opsx:explore` 后的参数就是用户想探索的内容，例如：
- 一个模糊想法："real-time collaboration"
- 一个具体问题："the auth system is getting unwieldy"
- 一个 change 名："add-dark-mode"（围绕该 change 的上下文展开探索）
- 一个对比问题："postgres vs sqlite for this"
- 什么都不写（直接进入 explore mode）
...
如果用户提到了具体的 change 名，就读取它的 artifacts 作为上下文。
...
当想法成型时，你可以给出一个总结，但这不是必须的。有时候，思考本身就是价值。
