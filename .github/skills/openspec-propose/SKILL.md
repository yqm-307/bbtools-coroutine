...
description: 提出一个新的变更，并一步生成所有 artifacts。当用户想快速描述要构建的内容，并获得可直接进入实现的 proposal、design、specs 与 tasks 时使用。
license: MIT
...
**输入**：用户请求中应包含一个 change 名称（kebab-case），或一段关于想构建内容的描述。

**步骤**

1. **如果没有提供清晰输入，先询问用户想做什么**

   使用 **AskUserQuestion tool**（开放式问题，不带预设选项）提问：
   > "What change do you want to work on? Describe what you want to build or fix."

   根据用户描述生成一个 kebab-case 名称（例如 "add user authentication" → `add-user-auth`）。
