...
description: 从 OpenSpec 变更中实现任务。当用户想开始实现、继续实现或按任务推进时使用。
license: MIT
...
1. **选择 change**
   ...
   如果提供了名称，直接使用。否则：
   - 如果用户在对话中提到过某个 change，则尝试从上下文推断
   - 如果只有一个活跃 change，则自动选择
   - 如果存在歧义，运行 `openspec list --json` 获取可用变更，并使用 **AskUserQuestion tool** 让用户选择

   始终明确告知："Using change: <name>"，并说明如何覆盖（例如 `/opsx:apply <other>`）。
...
3. **获取 apply 指令**
   ...
   **状态处理：**
   - 如果 `state: "blocked"`（缺少 artifacts）：展示说明，并建议使用 openspec-continue-change
   - 如果 `state: "all_done"`：告知已完成，并建议归档
   - 否则：进入实现流程
...
- **可在任意时机调用**：在所有 artifacts 完成前（如果任务已存在）、部分实现之后、或与其他动作交错进行时都可以调用
- **允许更新 artifacts**：如果实现暴露设计问题，应建议更新 artifacts，而不是被固定阶段锁死，应保持流动式工作
